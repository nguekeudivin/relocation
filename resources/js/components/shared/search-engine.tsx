        ...
        <section className="mb-4">
            <div className={cn('flex items-center gap-4', className)}>
                <div className={cn('relative', inputSize)}>
                    ...
                    />
                    <Search className="absolute top-3 right-3 h-4 w-4 text-gray-500" />
                </div>

                <div className="flex items-center gap-2">
                    {selects.map((item: any, index: number) => {
                        const options = item.placeholder ? [{ label: item.placeholder, value: undefined }, ...item.options] : item.options;
                        return (
                            <Select.Root
                            ...
                            >
                                <Select.Button className={cn('w-[200px]', item.size)} prefix={item.icon} label={<Select.Label options={options} />} />

                                <Select.Content className={item.size}>
                                    {options.map((item: any, index2: number) => (
                                        <Select.Item key={`filter${item.name}${index}${index2}`} value={item.value}>
                                            {item.label}
                                        </Select.Item>
                                    ))}
                                </Select.Content>
                ...
                {children && (
                    <Button
                        ...
                        color="outline"
                        className="border-1 border-gray-300 bg-white px-3 py-[0.6rem]"
                    >
                        <ListFilterPlus className="h-4 w-4" />
                    </Button>
                )}
            </div>
            {store.display.visible.TableFilters && children && <div className="mt-4 rounded-md border border-gray-200 bg-white p-4">{children}</div>}
        </section>
    );
};
