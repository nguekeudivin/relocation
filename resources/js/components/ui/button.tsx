...
const colorClasses: Record<NonNullable<ButtonProps["color"]>, string> = {
  default: "text-white bg-primary-500 hover:bg-primary-600 focus:ring-blue-300",
  secondary:
    "text-white bg-secondary-700 hover:bg-secondary-800 focus:ring-secondary-300",
...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  (
    { children, color = "default", className = "", loading = false, ...props },
    ref
  ) => {
    return (
      <button
        ...
        disabled={loading}
        className={cn(
          "text-sm font-medium  px-5 py-1.5 focus:outline-none focus:ring-4 inline-flex gap-1.5 items-center justify-center rounded-md",
          colorClasses[color],
          className
        )}
        {...props}
