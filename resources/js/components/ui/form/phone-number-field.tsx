...
export default function PhoneNumberField({
  ...
  useEffect(() => {
    ...
    const codeSearchResult = list.find(
      (item) => item.code == getParsed(value as string).code
    );
  ...
  const handleChange = (event: FormEvent) => {
    if (country)
      onValueChange(
        `${country.code}${(event.target as HTMLInputElement).value}`
      );
  };
  ...
  return (
    <div>
      <div
        ...
        className={clsx(
          cn(
            "relative flex h-10 items-center rounded-md border border-gray-300",
            className
          ),
          {
      ...
      >
        ...
        <div
          className={cn(
            "flex items-center border-r border-gray-300 p-2 pr-3",
            countryClassName
          )}
        >
          <div
        ...
        <input
          ...
          className={cn(
            "w-full border-none px-3 py-1.5 text-sm focus:border-none focus:outline-none",
            inputClassName
          )}
        />
        ...
        {value != undefined && value != "" && (
          <button
            ...
            className="absolute top-[20%] right-2 rounded-full p-1 hover:bg-gray-200"
          >
