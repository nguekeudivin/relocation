...
export const SelectField = ({
    ...
    return (
        <div className={cn('relative', className)}>
            {label && (
                <label htmlFor={generatedId} className="mb-2 block text-sm font-medium text-gray-900">
                    {label}
                </label>
            )}
            ...
            <select
                ...
                onBlur={onBlur}
                className={cn(
                    'border border-gray-300 bg-gray-50 text-sm text-gray-900',
                ...
                {...props}
            >
                {(addNone ? [{ label: noneLabel, value: '' }, ...options] : options).map((option, index) => (
